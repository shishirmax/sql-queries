CREATE PROCEDURE SP_GetDetailReportTest
--Parameter
--Declare
...
from Contact CO
join ADDRESS AD on CO.ADDRESS_ID = AD.ADDRESS_ID
join MASTER_BUSINESS_ENTITY MBE on CO.MASTER_BUSINESS_ENTITY_ID = MBE.MASTER_BE_ID
join BATCH_SCHEDULE_DETAIL BSD on MBE.MASTER_BE_ID = BSD.SHIPPER_ID
join MASTER_LOCATION ML on BSD.LOCATION_ID = ML.MASTER_LOCATION_ID
where BSD.COMMENT like '%K%' or BSD.COMMENT like '%M%'
and MBE.MASTER_BE_ID IN(Select BSD.SHIPPER_ID from BATCH_SCHEDULE_DETAIL)
and ML.MASTER_LOCATION_ID IN(select BSD.LOCATION_ID from BATCH_SCHEDULE_DETAIL)
